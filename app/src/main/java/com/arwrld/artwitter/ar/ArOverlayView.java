...
//            add(new ArPoint("Test", 0.0, 0.0, 0));
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        for (int i = 0; i < arPoints.size(); i++) {
            Status parseObject = arPoints.get(i).getStatus();
    ...
    private void drawAttr(Canvas canvas, Status status, int i) {

        Location location = new Location("ArLocation");
        location.setLatitude(status.getGeo().getCoordinates().get(1));
        location.setLongitude(status.getGeo().getCoordinates().get(0));
